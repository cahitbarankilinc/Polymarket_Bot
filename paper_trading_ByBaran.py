...
PRESET_CONFIG = {
    ...
    "fee_rate": 0.25,        # Polymarket docs: feeRate
    "fee_exponent": 2,       # Polymarket docs: exponent
    "fee_round_dp": 4,       # docs: fees rounded to 4 decimals
    "fee_min": 0.0001,       # docs: min non-zero fee
}
...
class PaperTrader:
    ...
    def calc_fee(self, price: float, size: float) -> float:
        # Polymarket docs: fee = C * feeRate * (p*(1-p))^exponent
        fee = size * self.config.fee_rate * (price * (1.0 - price)) ** self.config.fee_exponent

        # docs: rounded to 4 decimals
        dp = self.config.fee_round_dp
        fee = round(fee, dp)

        # docs: min non-zero fee 0.0001 (smaller -> 0)
        if 0 < fee < self.config.fee_min:
            return 0.0
        return fee
...
def config_from_preset() -> TraderConfig:
    ...
    return TraderConfig(
        ...
        side_mode=side_mode,
        fee_rate=PRESET_CONFIG["fee_rate"],
        fee_exponent=PRESET_CONFIG["fee_exponent"],
        fee_round_dp=PRESET_CONFIG["fee_round_dp"],
        fee_min=PRESET_CONFIG["fee_min"],
    )
