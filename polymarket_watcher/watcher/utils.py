...
def current_bucket_times_berlin(now_berlin: datetime) -> tuple[datetime, datetime]:
    if now_berlin.tzinfo is None:
        raise ValueError("now_berlin must be timezone-aware")

    remainder_minutes = now_berlin.minute % 15
    bucket_start = now_berlin - timedelta(
        minutes=remainder_minutes,
        seconds=now_berlin.second,
        microseconds=now_berlin.microsecond,
    )
    bucket_start = bucket_start.replace(second=0, microsecond=0)
    bucket_close = bucket_start + timedelta(minutes=15)
    return bucket_start, bucket_close


def current_bucket_close_time_berlin(now_berlin: datetime) -> datetime:
    _, bucket_close = current_bucket_times_berlin(now_berlin)
    return bucket_close
