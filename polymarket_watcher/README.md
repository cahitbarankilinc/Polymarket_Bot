...
Headful Playwright watcher that generates the current Bitcoin 15-minute event URL from the next Berlin bucket (0/15/30/45), reads the Up/Down button prices, and logs a Won/Lost verdict per event. The bot never clicks trade – it only records one line per event in `results.txt`.
...
- Event URL is produced as `https://polymarket.com/event/btc-updown-15m-<UTC_CLOSE_TIMESTAMP>`, where the timestamp is the UTC Unix seconds for the market close. Buckets advance by 900 seconds (15 minutes).
...
- Berlin time determines the next close bucket (minutes 0, 15, 30, 45). For example, 12:07 → 12:15:00 close; 12:15:01 → 12:30:00 close.
- The close time is converted to UTC, then to a Unix timestamp (seconds). That timestamp is appended to `BASE_EVENT_URL` (default `https://polymarket.com/event/btc-updown-15m-`).
- Buckets are 900 seconds apart, so each new market increments the trailing timestamp by 900.

## Output
- Results are appended to `results.txt` in the format (timestamp in Berlin time, event slug from the UTC close timestamp):
  ```
  2025-12-22T01:06:42+01:00 | btc-updown-15m-1734837300 | Won
  2025-12-22T01:21:42+01:00 | btc-updown-15m-1734838200 | Lost
  ```
- State of seen close timestamps is stored in `state.json` to avoid duplicate lines.
